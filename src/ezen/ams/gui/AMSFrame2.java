...
//		Enum 이용
	...
	private void printHeader() {
		accListTA.append("===================================================================\n");
		accListTA.append("계좌번호        예금주\t비밀번호\t잔액\t       대출금액\n");
		accListTA.append("===================================================================\n");
	}
	...
	public void allList() {
		accListTA.setText("");
		printHeader();
		List<Account> list = AMS2.repository.getAccounts();
		for (Account account : list) {
			accListTA.append(account.toString() + "\n");
		}
	}
	
//	public void min_List() {
//		accListTA.setText("");
//		printHeader();
//		List<Account> list = AMS3.repository.getAccounts();
//		System.out.println("Min_List 실행됨");
//		if(list instanceof MinusAccount) {
//			for(Account account : list){
//				accListTA.append(account.toString() + "\n");
//			}
//		}
//	}
	...
	/**
	 * 계좌번호or예금주에 해당하는 계좌 보여주기 
	 * 추가 : 2023-05-26
	 * 최종 수정일 : 2023-05-28 
	 * 수정 내용 : 각 TF에 알맞는 유효성 검사 메서드 추가.
	 */
	public void searchAcc(boolean category) {
		...
		if (category) {
			...
			if (accNumValid(accountNum)) {
				account = AMS2.repository.searchAccount(accountNum);
				accListTA.append(account.toString() + "\n");
			...
			if (ownerValid(ownerName)) {
				List<Account> ownerList = AMS2.repository.searchAccountByOwner(ownerName);
	...
	public void deleteAccount() {
		...
		if (accNumValid(accountNum)) {
			...
			if (answer == JOptionPane.YES_OPTION) {
				AMS2.repository.removeAccount(accountNum);
	...
	public void addAccount() {
		...
		if (ownerValid(accountOwner) && passValid(passCheck)) {
			...
			AMS2.repository.addAccount(account);
